...
knitr::knit_hooks$set(plot = function(x, options) {
    if (!is.null(options$fig.lab)) {
        paste('![', options$fig.cap, '](',
              ...
              '){#fig:', options$fig.lab, "}", sep = '')
    }
})

# knitr::knit_hooks$set(divid = function(before, options) {
#     if (before && !is.null(options$label)) {
#         paste0('<div id="', options$label, '">')
#     }
#     if (!before && !is.null(options$label)) {
#         paste0('</div>')
#     }
# })
# knitr::opts_chunk$set(divid = TRUE)
# knitr::knit_hooks$set(output = function(x, options) {
    # if(!is.null(options$label) && options$echo == TRUE) {
        # paste("<pre id=\"", options$label, "\">", x, "</pre>", sep = "")
    # } else {
        # paste("<pre>", x, "</pre>", sep = "")
    # }
# })
...
Six separate literature searches were conducted using the [_PsycINFO_](http://www.apa.org/pubs/databases/psycinfo/) and [_Web of Science_](http://wokinfo.com) online citation indexing databases via the [Portland State University library website](library.pdx.edu)<sup style="color:var(--link-color);font-weight:bold;">1</sup>:
`r tufte::margin_note("<sup style=\"color:var(--link-color);font-weight:bold;\">1</sup>Note that (1) \"intimate partner violence\" included \"domestic violence\" and \"partner abuse\", (2) \"same-sex\" included \"same-gender\", and (3) the results ranges provided after each search description listed reflect the minimum and maximum number of results returned across the two databases searched.")`
...
```{r src_dbsrch, opts.label='invisible'}
...
```{r dbsrch, results='asis', echo=FALSE}
...
```{r src_journals, opts.label='invisible'}
...
```{r journals, results='asis', echo=FALSE}
...
```{r src_MAPrqda, opts.label='invisible'}
...
```{r donut_scat, fig.cap="Proportions of reviewed articles in each of the two overarching research categories: IPV interventions research, and SMW-inclusive IPV research", fig.lab="donut_scat"}
### PLOT - scat - donut ####
...
```{r parset_scatXjournal, fig.fullwidth=TRUE}
...
pj <- mpal(1:length(unique(MAP$jrnl)))
library(ggparallel)
...
```{r hist_yrXscat, echo=FALSE, fig.fullwidth=TRUE, fig.cap="Publication Years Grouped by Research Category", fig.height = 4}
...
pd <- mpal(1:length(unique(ct.d$clab)))

ct.d <- dplyr::rename(ct.d, "Category" = scat, "Design" = clab)

```{r parset_designs, fig.fullwidth=TRUE}
### PLOT - designs - parset ####
...
pexp <- mpal(1:length(unique(ct.exp$clab)))

ct.exp <- dplyr::rename(ct.exp, "Category" = scat, "Experimental Design" = clab)

```{r parset_exp, fig.fullwidth = TRUE}
...
```{r parset_methodologies, fig.fullwidth=TRUE}
### PLOT - methodologies - parset ####

nlabs <- length(unique(ct.mo$clab))
pmo <- mpal(1:length(unique(ct.mo$clab)))
...
```{r parset_qlDesigns, fig.fullwidth=TRUE}
### PLOT - qual designs - parset ####

nlabs <- length(unique(ct.dql$clab))
pdql <- mpal(1:length(unique(ct.dql$clab)))
...
```{r parset_qlMethods, fig.fullwidth=TRUE}
### PLOT - qual methods - parset ####
pql <- mpal(1:length(unique(ct.ql$clab)))
...
```{r qlAnalytics, fig.fullwidth=TRUE, fig.height=7}
...
paql <- mpal(1:length(unique(ct.aql$clab)))
...
```{r parset_qlAnalytics, fig.fullwidth=TRUE}
...
```{r parset_qtDesigns, fig.fullwidth=TRUE}
### PLOT - quant designs - parset ####

nlabs <- length(unique(ct.dqt$clab))
pdqt <- mpal(1:length(unique(ct.dqt$clab)))
...
```{r parset_qtMethods, fig.fullwidth=TRUE, fig.height=6}
### PLOT - quant Methods - parset ####

nlabs <- length(unique(ct.qt$clab))
pqt <- mpal(1:length(unique(ct.qt$clab)))
...
```{r dot_qtAnalytics, fig.fullwidth=TRUE, fig.height=7}
### PLOT - quant analytics - dotchart ####

nlabs <- length(unique(ct.aqt$clab))
paqt <- mpal(1:length(unique(ct.aqt$clab)))
...
```{r parset_qtAnalytics, fig.fullwidth=TRUE, fig.height=7}
...
```{r parset_archivalDataSrcs, fig.fullwidth=TRUE}
### PLOT - archival/secondary data sources - parset####

nlabs <- length(unique(ct.rcrd$clab))
prcrd <- mpal(1:length(unique(ct.rcrd$clab)))
...
```{r parset_mixedMethods, fig.fullwidth=TRUE}
### PLOT - mm designs - parset ####

nlabs <- length(unique(ct.dmm$clab))
pmm <- mpal(1:length(unique(ct.dmm$clab)))
...
```{r parset_mmMethods, fig.fullwidth = TRUE}
### PLOT - mm methods - parset ####

nlabs <- length(unique(ct.mm$clab))
pmm <- mpal(1:length(unique(ct.mm$clab)))
...
```{r dot_populations, fig.fullwidth=TRUE, fig.height = 7.5}
### PLOT - populations - 1 ####
...
```{r dot_settings, fig.fullwidth=TRUE, fig.height = 7.5}
...
```{r parset_settings, fig.fullwidth=TRUE, fig.height = 7.5}
...
pset2 <- mpal(1:length(unique(ct.set2$clab)))
...
```{r parset_sampling, fig.fullwidth=TRUE}
### PLOT - samplingMethods - parset ####

nlabs <- length(unique(ct.smthds$clab))
psmthds <- mpal(1:length(unique(ct.smthds$clab)))
...
```{r parset_ecoLvls, fig.fullwidth=TRUE}
### PLOT - eco levels - parset ####

nlabs <- length(unique(ct.eco$clab))
peco <- mpal(1:length(unique(ct.eco$clab)))
...
```{r net_lvls_bibkeys, fig.fullwidth=TRUE}
# PLOT - `llongg` ----------------
...
```{r arc_lvls, fig.height=3.75, fig.fullwidth=TRUE, out.width="1002px", out.height="550px"}
...
```{r arc_sys, fig.height=3.5, fig.fullwidth=TRUE}
...
```{r arc_alnetg, out.height='4in', fig.cap=alnetg_cap}
...
```{r arc_aledges2, fig.show='asis', fig.cap="Network Diagram Showing Relations among Analytic Approaches (numbered graph nodes) used and Ecological Levels of Analysis (named graph nodes) Invovled among the Reviewed Literature", fig.height=8, fig.fullwidth=TRUE}
...
```{r tl_map, fig.fullwidth=TRUE, fig.height=4, out.width='\\linewidth', fig.align='center'}
...
```{r tl_inv, fig.fullwidth=TRUE, fig.height=2.75, out.width='\\linewidth'}
...
```{r tl_smw, fig.fullwidth=TRUE, fig.height=2, out.width='\\linewidth', fig.show='asis'}
