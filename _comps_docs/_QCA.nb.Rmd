---
title: "Qualitative Comparative Analyses Source Code"
---
...
# cf_button <- ""
# knit_hooks$set(codeFold = function(before) {
#     if (before) {
#         cat("<button type='button' class='btn btn-default btn-xs code-folding-btn pull-right' data-toggle='collapse' data-target='#rcode-643E0F361' aria-expanded='true' aria-controls='rcode-643E0F361'><span>Hide</span></button>")
#         })
#     })
# knit_hooks$set(button_cf = function(before, options) {
#     if (options$echo == TRUE) {
#         opts_knit$set(codeFold = TRUE)
#     } else {
#         opts_knit$set(codeFold = FALSE)
#     }
#     })
```
...
```{r journals2, results='asis', echo=FALSE}
j.vp %>% as.list() %>% pander()
```
...
```{r donut_scat, fig.cap="Proportions of reviewed articles in each of the two overarching research categories: IPV interventions research, and SMW-Specific IPV research", fig.lab="donut_scat", fig.fullwidth=TRUE}
### PLOT - scat - donut ####
library(ggplot2)
...
ft.jrnl %>% kable(caption = "Number of Publications in Each Research Category per Journal", align = c("l", "r", "r"), col.names = c("Publication Title", "Category"))
...
```{r hist_yrXscat, echo=FALSE, fig.fullwidth=TRUE, fig.cap="Publication Years Grouped by Research Category", fig.height = 4, figPath=TRUE}
par(pmar)
...
```{r tl_map, fig.fullwidth=TRUE, fig.width=7, out.width='\\linewidth', fig.align='center', figPath=TRUE}
# PLOT - MAPtl ----------------
...
```{r tl_inv, fig.fullwidth=TRUE, fig.height=2.75, out.width='\\linewidth', figPath=TRUE}
...
```{r tl_smw, fig.fullwidth=TRUE, fig.height=2.75, out.width='\\linewidth', figPath=TRUE}
...
```{r dot_topics, fig.fullwidth=TRUE, fig.height = 7.5, figPath=TRUE}
...
kable(ktop[, 1:11], caption = "Primary Topics by Study (1/2)") %>%
    add_footnote("SMW-Specific", notation = "symbol")
cat("<span class='tnote'>")
pander(lvl.top[1:11])
cat("</span>")
```
...
```{r dot_qlAnalytics, fig.fullwidth=TRUE, fig.height=7}
...
```{r parset_qlAnalytics, fig.fullwidth=TRUE, figPath=TRUE}
...
```{r parset_qtAnalytics, fig.fullwidth=TRUE, fig.height=7,figPath=TRUE}
...
```{r dot_populations, fig.fullwidth=TRUE, fig.height = 7.5, figPath=TRUE}
### PLOT - populations - 1 ####
...
```{r dot_settings, fig.fullwidth=TRUE, fig.height = 7.5, figPath=TRUE}
...
```{r dot_sampling, fig.fullwidth=TRUE, figPath=TRUE}
...
# Levels of Analysis Involved in The Included Literature
...
```{r net_lvls_bibkeys, fig.fullwidth=TRUE, figPath=TRUE}
# PLOT - `llongg` ----------------
par(mar = rep(0, 4))
...
```{r net_lvls_keys_bibkeys_mat, dev="png", fig.retina=6, fig.fullwidth=TRUE, fig.cap=sprintf("Ecological Network: Levels of Analysis x Case. Group memberships derived from %s [@brandes2008on]", c("communality clustering algorithm which maximizes modularity across all possible partitions of the graph data in order to calculate the optimal cluster structure for the data", "eigenvalues computed for each case on each level of analysis presented in the matrix")), figPath=TRUE}
...
```{r arc_lvls_TEST, fig.height=3.75, fig.fullwidth=TRUE}
# PLOT - `lnet - arcplot()` ----------------
par(mar = c(5, 0, 0, 0))
...
```{r net_sysLvls, fig.fullwidth=TRUE, figPath=TRUE}
...
```{r tlnetg_cap, echo=FALSE}
tlabs <- gsub("&", "&", tlabs)
tlabs <- paste(tlabs, collapse = ", ")
tlnetg_cap <- "Network Diagram Showing Relations among Substantive Research Topics (numbered graph nodes) Covered & Ecological Levels of Analysis (named graph nodes) Involved among the Reviewed Literature"
# PLOTS - `tlnetg` (layout-0 & layout-2) ----------------
```

```{r net_topics, out.height='4in', fig.fullwidth=TRUE, fig.cap=tlnetg_cap, figPath=TRUE}
par(mar = rep(0, 4))
...
```{r arc_aledges2, fig.show='asis', fig.cap="Network Diagram Showing Relations among Analytic Approaches (numbered graph nodes) used and Ecological Levels of Analysis (named graph nodes) Invovled among the Reviewed Literature", fig.height=8, fig.fullwidth=TRUE, figPath=TRUE}
...
<span class="tnote">`r allabs1`</span>
...
```{r hclust_topics1, fig.fullwidth=TRUE, figPath=TRUE}
...
```{r hclust_topics2, fig.fullwidth=TRUE, figPath=TRUE}
...
```{r hclust_topics3, fig.fullwidth=TRUE, fig.cap="3-Group K-Means Clustering Solution for Primary Topics Covered Across the Included Literature", figPath=TRUE}
library(cluster)
palette(pal_my)
par(mar = c(0,2,0.7,0))
