getFigs <- function(f, figPath, ext = '.pdf', cap = NULL) { ## only works when the figure labels used in the text match the base filenames of the figures, and the figure files can all be found via the same base file path ##
    f <- readLines(f)
    p <- "\\{\\+@fig\\:.*?\\}"
    m <- regexpr(p, f)
    figRefs <- regmatches(f, m)
