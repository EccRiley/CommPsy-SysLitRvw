...
Collectively, the above-described literature searches yielded `r nrow(MAP.cpv)` reviewable journal articles, of which `r nrow(MAP.cpv[MAP.cpv$scat == "S3", ])` focus on intimate partner violence interventions, while the remaining `r nrow(MAP.cpv[MAP.cpv$scat == "S4", ])` consist of the available IPV-related research specific to LGBTQ populations within the set of above-described journals.
...
with(MAP.cpv, {ftable(journal, scat)}) %>% pander(caption="")
...
```{r }
Rtdf(MAP.cpv$journal, names = c("Publication Title", "$N_{Articles}$")) %>%
